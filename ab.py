...
def compute_and_export_from_bbox(minx, miny, maxx, maxy, farm_id: str) -> dict:
    """
    ...
    aoi = get_geometry_from_bbox(minx, miny, maxx, maxy)
...
def get_geometry(lat, lon, size_km=1):
    half_side = size_km * 500  
    geom = ee.Geometry.Rectangle([lon - 0.01, lat - 0.01, lon + 0.01, lat + 0.01])
    return geom
...
def compute_pca_stress(ndvi, ndre, ci, aoi):
    ...
    ndvi_weight = eigen_vectors.get([0, 0])
    ndre_weight = eigen_vectors.get([1, 0])
    ci_weight = eigen_vectors.get([2, 0])
